...
module.exports = ({ field, TC, managedModelType }) => TC.get('$removeById').addArgs({
  managedId: 'String!',
        ...
        if (Array.isArray(_field)) {
          ...
          if (exist) {
            ...
            try {
              ...
              return Error(`Unexpected error adding the document to ${scope.toLowerCase()}`);
            }
          } else {
            return Error(`This ${scope.toLowerCase()} cannot delete this document`);
          }
        } else {
